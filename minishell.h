...
void					handle_parent_fd(int *tmp_in, int pip_fd[2],
							t_cmdarg *current_cmd);
int						create_pipe_safe(int pip[2]);
int						ft_fork_and_exec(t_cmdarg *cmd, t_list *env,
							int *tmp_in, pid_t *last_cmd_pid);
// builtin_cd
void					update_pwd_vars(t_list *env, char *new_path,
							char *old_path);
char					*expand_tilde(char *cmd, t_list *env);
int						resolve_cd_target(char **cmd, char **path,
...
void					update_existing_env(t_list *dup_key, char **key,
							char **value, char *cmd);
// builtin_export_utils1
int						print_export_identifier_error(const char *cmd);
void					handle_export_no_assign(char **key, char **value,
							const char *cmd);
void					append_to_env_value(t_list *dup_key, char **key,
							char **value);
int						parse_export_argument(const char *cmd, char **key,
							char **value);
// builtin_export
...
int						open_file_for_builtin(char *filename,
							t_token_type type);
...
void					print_error_exit(const char *cmd_name,
							const char *error, int status);
int						safe_open(const char *file, int flag);
void					execve_error_cleanup(char **cmd_path, char **cmd_name,
							char **envp);
...
void					exec_external_command(t_cmdarg *current_cmd,
							t_list *env);
void					exec_builtin_in_child(t_cmdarg *current_cmd,
							t_list **env);
void					exec_child_process(t_cmdarg *current_cmd, t_list *env,
...
void					rd_hr_in_gl(char *delim, int fd_pipe[2],
							t_redi_list *herc, t_list *env);

// heredoc.c
int						heredoc_child_process(t_redi_list *heredoc,
							int fd_pipe[2], t_list *env);
void					heredoc_parent_finalize(int fd_pipe[2], pid_t pid,
							int *status, t_redi_list *in);
int						handle_single_heredoc(t_redi_list *in, int fd_pipe[2],
							t_list *env);
int						process_all_heredocs(t_cmdarg *shell, t_list *env);
int						setup_builtin_io(t_cmdarg *shell, int *input_fd,
							int *output_fd);

// utils.c
int						size_list(t_cmdarg *node);
void					free_all(char **bf, int j);
void					ft_alloc(char **envp, int *i, char *key_equals,
							t_list *env);
...
void					expand_norm(t_token **token, t_list *minienv,
							ssize_t dpos);
...
t_token					*ft_jointok(t_token *token, t_lexer **lexer,
							t_list *minienv, bool *heredoc);
t_token					*get_next_token(t_lexer *lexer, t_list *minienv,
							bool *heredoc);
...
char					*expand_inchar(char *value, t_list *minienv,
							ssize_t dollar_pos);
